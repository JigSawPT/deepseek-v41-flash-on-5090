...
The long form of the report: what the port had to change, every experiment with its result (the
negative ones included), and the lessons kept for the next model. Written from the day-by-day
record; dates are 2026. Two terms used throughout: **the probe** is the vendor's reference
implementation (Python, TileLang kernels) run on the same card through a two-tier expert cache
written for it — the first thing that ran, and the numerical reference for everything after;
**the port** is the llama.cpp fork branch.
...
7. **The hyper-connection mix is threaded one sub-layer ahead.** (Hyper-connections are V4's
   multi-stream residual: four copies of the residual stream, mixed by learned coefficients before
   and after every sub-layer.) Each sub-layer computes the coefficients the *next* one applies; the
   final collapse uses the mix the last FFN produced. V4 computes in place and has dedicated
   `hc_head` weights. The reference's docstring is the proof; the switch `LLAMA_DSV41_NO_HC_THREAD`
...
The plan had two routes — the probe first, the port second — plus a verification block and a list
of hypotheses for speed. Every item below has a measurement; the ones that failed are kept because
they close a door.

### On the probe

- **Kernel utilisation.** The batch-1 expert GEMM ran at 5.2 % of the card's bandwidth (200 µs per
  18.8 MB expert against 10.5 µs at peak). This reordered the plan: the "20 tokens/s software
  ceiling" was a 19× inefficiency, and llama.cpp's `mul_mat_id`, which delivers 72 % at n = 1, was
  the route.
- **Twelve defects** in the probe's cache (heat decay 40× per token, a host round-trip per expert,
...
- **A sequential trace** of every expert access with request, completion and need times — the
...
- **Fusing the gate and up projections is exact and gives nothing** — the kernel was not the limit.
- **Layer-ahead routing prediction works** (`gate_{L+1}(h_L)`: recall@6 70.9 %, 58.6 % of misses
  ...
  compute. Shelved, then reconsidered for the port, where disk is 94 % of the token — and finally
  ruled out by the prefetch-oracle curve (below).
- **More residency does not give throughput** on the probe: two independent experiments, +0 %
  and +0.5 %.
- **The GGUF conversion is a bijection**, 480/480.
- **Non-reproducibility.** Identical greedy runs gave different texts. Eleven suspects excluded
  with proof on the probe; the cause was found on the port (below).

### On the port

- **First light**, then the nine fixes above, then the q-norm.
- **The host-tier sweep** (pinned RAM tier 0 → 88 GiB): optimum at 72 GiB; 88 is slower because
  it steals page cache from the engram tables.
- **The benchmark on the port:** 5.12 tokens/s cold, 21.27 resident. The resident number is the
  compute ceiling, and the "discard the first request" convention would have published it as the
  conversation number.
- **Working set and eviction policy.** At 72 GiB, LRU, LFU, heat, random and MIN/Belady give the
  same misses: policy has 0 % of margin. 86.7 % of logical reads are physical. The working set grows
  26 experts per token in the tail: 105 GiB at 128 tokens.
- **Where the missing bandwidth is.** The NVMe delivers 10.04 GB/s at queue depth 6; the port
...
- **Prefix cache** was already delivered by the server: 32× on time to first token.
- **Prefetch oracle.** A speculative queue fed by a recorded trace: +30 % at 200 remap calls
  ...
  arrives. This rules out every layer-level predictor.
...
- **Engram prefetch:** 10.44 → 0.92 ms per token, 11.4×, exact.
- **Encoder-only prefill:** worth 23.9 % of prefill bytes, but not exact below 2 432 tokens (the
  receptive field of 19 sliding windows of 128), and the reference does not implement it. Closed.
...
- **n-gram draft:** null best case (+2 %), −33 % realistic; and it exposed the rollback gap
  (`need_n_rs_seq`) — fixed for every draft type.
- **DSpark draft head:** exported (78 tensors, 7.97 GB), acceptance 51–97 % by content, neutral
  on the benchmark (−4 % cold, 0 % resident), +12–15 % on verbatim repetition only; best draft
...
50. On the Hub, a lossless repack of published weights is almost free to upload: content-level
    deduplication moved about 2 GB of new data for a 502 GB file.
