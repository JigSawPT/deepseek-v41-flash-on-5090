...
| | |
|---|---|
| **Fit for use as the V4.1 engine** | Yes. Numerical gate passed on five criteria; V4 still runs on the same fork; configuration measured point by point. |
| **Fit for publication** | Yes, with five findings of general value. The speculative prefetch queue stays an instrument, not a feature. |
...
1. [Correctness: one real defect, then the floor](#correctness-one-real-defect-then-the-floor)
2. [Speed: where the token goes](#speed-where-the-token-goes)
3. [The deficit is in the graph, not the queue](#the-deficit-is-in-the-graph-not-the-queue)
4. [Speculative decoding with the model's own draft head](#speculative-decoding-with-the-models-own-draft-head)
5. [Recommended configuration, and what not to do](#recommended-configuration-and-what-not-to-do)
6. [The plan, item by item](#the-plan-item-by-item)
7. [Method notes](#method-notes)
...
input and returning a divergent output. A new `--dump` option in `llama-logits` writes any node the
graph already names through `cb()`, with no extra instrumentation. Two sub-steps in, the culprit
was `q`:
...
— the short gate never exercised it. No steps at layer 2 or layer 20, equal argmax, top-6 in the
...
The house ruler applied to the port (`tools/bench_server.py`), with the model resident in a
`llama-server` and four prompts of mixed content, three rounds each:
...
| Python probe, for comparison | 4.34–4.67 | 29 s |
...
8 906 and never climb again. **21.27 tokens/s is the compute ceiling, measured.** Applied to the
letter — "discard the first request" — the house ruler would publish 20.1 tokens/s, four times what
a conversation sees: that rule was written for models where the start-up cools, not the content.

![Stall decomposition per remap call at L2 = 72 GiB: compute 20 %, PCIe 26 %, NVMe 54 %](figures/stall.svg)

*The L2-tier ladder swept RAM with the VRAM cache fixed; the VRAM hit rate stayed invariant
(62.74 %) across the ladder, which is what makes the decomposition legitimate
(`tools/stall_decomposition.py`).*

| ceiling | tokens/s | how |
|---|---:|---|
| today, L2 = 72 GiB | 4.3 | expert-tier ladder |
| perfect look-ahead of 5 tokens | **5.6** | oracle A4, measured |
...
A speculative queue was built into `moe-stream`: it fills only the RAM tier, drains only when the
demand queue is empty, and is fed by a recorded trace of the run itself. An adversarial review
(68 agents, five dimensions, three sceptics per finding) caught seven defects — three of which
corrupted the readings — and after the fixes the curve looks like this:
...
**+30 % at the peak, five tokens ahead.** The model predicted where the ladder would stop: built on
the L2 ladder, it said the stall without disk misses would be 4.04 ms; measured at 200, 4.13 ms.
The same curve rules out a layer-level predictor: the depths such a predictor can reach (1, 2, 4
calls) cost **−19 / −7 / −16 %**, because the speculative read is still in flight when the demand
arrives and is paid twice. The gain lives at one token or more of look-ahead, and only knowing the
future tokens gets there — which is speculative decoding.
...
concentration only bites near saturation — and at K ≥ 8 the experts per layer exceed the 25 slots
and the multi-pass path kicks in. The consequence for speculative decoding on this machine: its
value comes almost entirely from tokens accepted per step, not from cheap verification.

### The one place where look-ahead works without a predictor

The n-gram memory (engram) addresses are computed from token ids, not from the hidden state — they
are known the instant the token exists. `set_input` read 56 rows per token, one at a time, on the
thread the graph was waiting for. Two passes — the first only hashes and cannot miss; between the
two, the whole list goes to the OS through `PrefetchVirtualMemory` — and:
...
per-head q norm, the hyper-connection mix threaded one sub-layer ahead, and a final collapse with
the last FFN mix rather than dedicated `hc_head` weights. A new converter class exports the head
(78 tensors, 7.97 GB) and a `dsv41_semantics` key tells the loader which rules apply. The head
loads, generates correct text, and reports its own acceptance (`tools/bench_dspark.py`; draft
experts in RAM so the target keeps its 18 GiB cache and the control is the same target):
...
Those two prompts are the mechanism's best and worst case. The house ruler — four prompts of mixed
content, three rounds each, same day, same configuration — is the number that counts:

| house ruler, draft of 2 | acceptance | cold, control | cold, draft | Δ | resident, control | resident, draft |
|---|---:|---:|---:|---:|---:|---:|
...
and the ruler's median is −4 %. The mechanism is not at fault: in a compute-bound regime, where a
batched verification is nearly free, the same acceptance would multiply throughput. That regime
...
```
--moe-stream --moe-stream-cache 18 --moe-stream-l2 72   # default I/O threads; no JigSaw pinning
...
| lever | status | why |
|---|---|---|
| L2 tier above 72 GiB | worse | 88 GiB gives 3.9–4.1: it steals page cache from the engram tables, and the tier's own hit rate rises while throughput falls |
...
| JigSaw pinning 7 experts | null | three independent measurements; at 72 GiB everything fits and there is nothing to pin |
| eviction policy | 0 % | LRU, LFU, heat, random and MIN/Belady give the same misses at 72 GiB (`tools/cache_oracle.py`) |
...
| encoder-only prefill (CED) | not exact | worth 23.9 % of prefill bytes, but the receptive field of the 19 sliding windows requires the decoder on the last 2 432 positions; below that the exact saving is zero, and the reference does not implement it |
| DSpark draft head, draft of 2 | neutral | house ruler −4 % cold, 0 % resident; −14 to +8 % per prompt, tracking 51–79 % acceptance; +12–15 % only on verbatim repetition |
...
| block | done | of | |
|---|---:|---:|---|
| Route A — the Python probe | 10 | 10 | complete, including A2 and A5 closed with a measured negative |
| Route B — the port | 8 | 8 | complete; 7 built, CED closed with an answer |
| Verification | 4 | 4 | complete |
| Hypotheses | 5.75 | 7 | H1, H4, H9 done; H8 for free (32× on TTFT from the server's prefix cache); H2 measured on both drafts (n-gram negative, DSpark neutral); H5 half |
| **total** | **27.75** | **29** | **96 %** — excluding H6, H7 and KV-fp4, which quantize |
...
- **A hypothesis must not evaluate itself.** The speculative probe used `find()`, which counts a
  hit and sets the CLOCK bit — inflating the statistic it was about to be judged by.
...
- **Repeating the prompt does not warm the engine — it warms the answer.** The house rule "discard
  the first request" measures the cache on models whose residency depends on the content.
- **"The disk is not the limit" is a conclusion about an engine, not about a model.** The Python
  probe spent 20 % of the token on disk; the port spends 94 %.
