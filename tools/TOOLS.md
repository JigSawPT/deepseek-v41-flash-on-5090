...
The measurement tools behind the numbers in the report. They are the subset of our toolbox that
produced published figures, rewritten in English with paths as arguments; the rest (the harness
that runs the vendor's reference implementation with a two-tier expert cache — "the probe" in
the report — the trace recorders, the host-tier sweep scripts) stays private because it depends
on our infrastructure. Where a tool needs an input that only the probe can produce, it says so.
...
|---|---|---|
| `bench_server.py` | the benchmark of the report on a resident `llama-server`: four prompts, three rounds, TTFT from streaming, decode between first and last piece | 5.12 / 21.27 tokens/s; the ruler with the draft |
| `read_bench.py` | reads the benchmark JSON separating cold (first round) from resident (later rounds) — the two are different quantities on this model | the cold/resident table |
...
| `stall_decomposition.py` | compute / PCIe / NVMe share of a remap call from the host-tier sweep; validates against the resident case | 20 / 26 / 54 %; the 6.2 and 21.3 ceilings |
...
Result files in [`../results/`](../results/): the benchmark JSONs (`bench_server_baseline.json`,
...
produced by our original Portuguese-named tools and renamed to the field names used here by
